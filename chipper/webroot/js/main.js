...
function generateXiaozhiPairingCode() {
  const deviceId = document.getElementById("deviceIdInput").value || "unknown_device";
  ...
  const url = `/api/xiaozhi_generate_pairing_code?device_id=${encodeURIComponent(deviceId)}`;
  ...
  fetch(url, { method: "POST" })
    ...
    .then(data => {
      if (data.code) {
        ...
        statusDiv.innerHTML = "Pairing code generated successfully!";
        statusDiv.style.color = "#0f9d58";
        
        // Update expiry info
        const expiresIn = data.expires_in || 600;
        infoDiv.textContent = `Device ID: ${data.device_id} | Expires in: ${expiresIn} seconds`;
        ...
        const countdownInterval = setInterval(() => {
          ...
          if (remaining > 0) {
            ...
            infoDiv.textContent = `Device ID: ${data.device_id} | Expires in: ${minutes}:${seconds.toString().padStart(2, '0')}`;
          } else {
            clearInterval(countdownInterval);
            codeDisplay.style.display = "none";
            statusDiv.innerHTML = "Pairing code has expired. Please generate a new one.";
            statusDiv.style.color = "#db4437";
          }
        }, 1000);
      } else {
        statusDiv.innerHTML = "Failed to generate pairing code.";
        statusDiv.style.color = "#db4437";
      }
    })
    .catch(error => {
